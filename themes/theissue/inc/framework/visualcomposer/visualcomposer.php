	...
	if (function_exists('visual_composer')) {
      ...
      return $param_line;
		...
		function thb_hotspot_image( $param, $value ){
			...
			$uni = uniqid('thb-hotspot-'.rand());
			$output = '<div class="thb-hotspot-param-container">';
			$output .= '<div class="thb-hotspot-image-holder no-img" data-popup-title="'.esc_attr__('Hotspot Tooltip Content', 'theissue').'" data-save-text="'.esc_attr__('Save changes', 'theissue').'" data-close-text="'.esc_attr__('Close','theissue').'"></div>';
