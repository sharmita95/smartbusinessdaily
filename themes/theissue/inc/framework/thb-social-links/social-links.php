...
class THB_Social_Links {
  ...
  */
  ...
  public function thb_get_medium($username) {
		...
		// Check user
		if ( !$username ) {
			$result['error'] = esc_html( 'Please enter a Medium User.', 'theissue' );
			return $result;
		}
  ...
  public function thb_get_youtube($username) {
    ...
    // Check username
    if ( !$username ) {
      $result['error'] = esc_html( 'Please enter an YouTube User.', 'theissue' );
      return $result;
    }
    ...
    if ( isset( $data->error->message ) ) {
      ...
      // API Error
      $result['error'] = esc_html( 'Please check your username or channel IDs.', 'theissue' );
  ...
  public function thb_get_twitter( $username ) {
    ...
    // Check username
    if ( !$username ) {
      $result['error'] = esc_html( 'Please enter a Twitter User.', 'theissue' );
      return $result;
    }
    ...
    if ( ! $data ) {
      ...
      if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
        ...
        if ( ! isset( $data->errors ) ) {
          $result['error'] = esc_html( 'Not Available', 'theissue' );
        }
      } else {
        $data = json_decode( wp_remote_retrieve_body( $response ) );
      }
  ...
  public function thb_get_pinterest($username) {
    ...
    // Check username
    if ( ! $username ) {
      $result['error'] = esc_html( 'Please enter a pinterest user name.', 'theissue' );
      return $result;
    }
  ...
  public function thb_get_facebook($username) {
    ...
    // Check username
    if ( !$username ) {
      $result['error'] = esc_html( 'Please enter a Facebook user name.', 'theissue' );
    ...
    if ( $fb_result ) {
      ...
      $result['error'] = esc_html__( 'Data not found. Please check your user ID.', 'theissue' );
    }
  ...
  public function thb_get_instagram($username) {
    ...
    // Check Username
    if ( ! $username ) {
      $result['error'] = esc_html( 'Please enter an Instagram Username.', 'theissue' );
      return $result;
    }
